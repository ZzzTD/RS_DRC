...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    elif netG == 'ResnetGenerator_swin':
        net = ResnetGenerator_swin(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=9, opt=opt)
...
class pdfam_gau_module1(torch.nn.Module):
    def __init__(self, channels=1):
        super(pdfam_gau_module1, self).__init__()
...
class pdfam_gau_module2(torch.nn.Module):
    def __init__(self, channels=1):
        super(pdfam_gau_module2, self).__init__()
        self.learn_eps = nn.Parameter(torch.zeros([1, channels, 1, 1]))
        self.learn_coef = nn.Parameter(torch.ones([1, channels, 1, 1]))
        self.learn_width = nn.Parameter(torch.ones([1, channels, 1, 1]))
        self.act = nn.Softplus()
...
class S_APE(nn.Module):
    def __init__(self):
        super(S_APE, self).__init__()

        [self.b, self.c, self.h, self.w] = [1,192,64,64]
    ...
    def forward(self,x):
        ...
        x = x.view(self.b, self.c, self.h * self.w).transpose(1, 2).contiguous()  # b hw c
        position_embeddings = position_embeddings.view(self.b, self.c, self.h * self.w).transpose(1, 2).contiguous()

        x = self.drop(x + position_embeddings)  # [b,hw,c]
        x = x.permute(0, 2, 1).reshape(self.b, self.c, self.h, self.w)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self,x):
        ...
        if self.shift_size > 0:
            ...
            attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0)).to(
                x.device)
        else:
            attn_mask = None
...
# 下面这个类对应着swintransformerblock提取完特征后的变换
...
class W_i_Block(nn.Module):
    def __init__(self):
        super(W_i_Block,self).__init__()
        stb = [SwinTransformerBlock(dim=192, num_heads=12, shift_size=0),
               SwinTransformerBlock(dim=192, num_heads=12, shift_size=3)]
        self.stb1 = nn.Sequential(*stb)
        self.stb2 = nn.Sequential(*stb)
        self.stb3 = nn.Sequential(*stb)
    def forward(self,x):
        wj0 = x
        wj1 = self.stb1(x)
        wj2 = self.stb2(x)
        delta_wj1 = wj1-wj0
        delta_wj2 = wj2-wj1
        x_out = wj2+(7/2)*delta_wj2-(11/2)*delta_wj1+(3/2)*self.stb3(x)
        return x_out

class ResnetGenerator_swin(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        """
        assert(n_blocks >= 0)
        super(ResnetGenerator_swin, self).__init__()
        self.opt = opt
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
        ...
        model += [nn.Conv2d(gf_dim, gf_dim * 2, 3, 2, padding=1),  # 2dim*128*128
                 nn.InstanceNorm2d(gf_dim * 2),
                 nn.LeakyReLU()]

        model += [nn.Conv2d(gf_dim * 2, gf_dim * 4, 3, 2, padding=1),  # 4dim*64*64
        ...
        # (2,48,128,128)
        model += [nn.Conv2d(gf_dim*4, gf_dim*2, 1, 1, padding=0),
                  nn.InstanceNorm2d(gf_dim * 2),
                  nn.LeakyReLU(),
                  nn.Upsample(scale_factor=2, mode="bilinear"),
                  nn.Conv2d(gf_dim * 2, gf_dim , 3, 1, padding=1),  # 2dim
                  nn.InstanceNorm2d(gf_dim),
                  nn.LeakyReLU()]
        # (2,12,256,256)
        model += [nn.Conv2d(gf_dim*1, int(gf_dim/2), 1, 1, padding=0),
                  nn.InstanceNorm2d(int(gf_dim/2)),
                  nn.LeakyReLU(),
                  nn.Upsample(scale_factor=2, mode="bilinear"),
                  nn.Conv2d(int(gf_dim/2), int(gf_dim/4), 3, 1, padding=1),  # dim
                  nn.InstanceNorm2d(int(gf_dim/4)),
                  nn.LeakyReLU(),]
        # (2,6,256,256)
        model += [nn.Conv2d(int(gf_dim/4), int(gf_dim/8), 1, 1, padding=0),
                 nn.InstanceNorm2d(int(gf_dim/8)),
                 nn.LeakyReLU()]

        # (2,3,256,256)
        model += [nn.ReflectionPad2d(3),
                  nn.Conv2d(int(gf_dim/8), input_dim, 7, 1),
                  nn.Tanh()]
            ...
            for layer_id, layer in enumerate(self.model):
                ...
                if layer_id in layers:
                    ...
                    if len(feat.shape)==4:
                        ...
                        tmp_feat = feat.transpose(1,2).reshape(1, 192, 64, 64)
