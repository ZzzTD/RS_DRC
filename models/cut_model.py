...
class CUTModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'D_real', 'D_fake', 'G', 'NCE','H_s','SRC','G_s_idt_B']
        # self.loss_names = ['G_GAN', 'D_real', 'D_fake', 'G', 'NCE','SRC']
        ...
        if self.isTrain:
            ...
            self.criterionFeature = losses.PerceptualLoss().to(self.device) # vgg感知损失
    ...
    def forward(self):
        ...
        '''由于swin的位置编码无法确定batch，因此在这里统一改为batch为1'''
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_NCE > 0.0:
            ...
            # 3. 加SRCloss
            self.loss_SRC = self.calculate_R_loss(real_A_pool, fake_B_pool)
        else:
            self.loss_NCE, self.loss_NCE_bd = 0.0, 0.0
        if self.opt.nce_idt and self.opt.lambda_NCE > 0.0:
            norm_fake_idt_B = self.normalization((self.idt_B + 1) * 0.5)
            norm_real_B = self.normalization((self.real_B + 1) * 0.5)
            self.loss_G_s_idt_B = self.Spatial_Loss(self.netPre, norm_real_B, norm_fake_idt_B, None)


        norm_real_A = tensor2pdf(self.real_A)
        norm_fake_B = tensor2pdf(self.fake_B)
        self.loss_H_s = self.Spatial_Loss(self.netPre, norm_real_A, norm_fake_B, None)

        self.loss_G = self.loss_G_GAN + self.loss_NCE *0.5 + self.loss_SRC *0.5 + self.loss_H_s *5  + self.loss_G_s_idt_B*35
